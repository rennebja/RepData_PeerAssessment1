...
The 5-minute interval showing the greated average activity is **`r maxact`**

## Imputing missing values

Let's take another look at the total number of steps per day. There were quite a lot of missing values in the dataset. `r sum(is.na(activity$steps))` missing values to be exact.
...
As can be seen, imputing the missing values by using average numbers has shifted the median little from our first value `r datetotmed` to this one `r as.integer(idatetotmed)`. The mean values have shifted more significantly with our first value of `r round(datetotmean)` appreciably below the median while this mean value equals the median.
